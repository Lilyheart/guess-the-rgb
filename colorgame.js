...
function rgbTOhsl(rgb) {
  ...
  if (max === min) {
    ...
    switch (max) {
      case redRange:
        hue = ((greenRange - blueRange) / (max - min)) * DEGREES_CALC;
        break;
      case greenRange:
        hue = (GREEN_HUE_CALC + (blueRange - redRange) / (max - min)) * DEGREES_CALC;
        break;
      default:
        hue = (BLUE_HUE_CALC + (redRange - greenRange) / (max - min)) * DEGREES_CALC;
    }
    if (hue < 0) {
      hue += FULL_CIRCLE;
    }
...
function settingListeners() {
  ...
  for (i = 0; i < modelButtons.length; i += 1) {
    modelButtons[i].addEventListener('click', changeModel);
  }
}
